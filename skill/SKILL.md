...
name: skill-audit
category: quality
description: "DSH 技能的审核通道：以确定性脚本对技能做静态体检——frontmatter 契约、脚本可用性（UTF-8 BOM + PowerShell 5.1 可解析）、SKILL.md 引用完整性、敏感信息与凭据泄漏、机器专属路径、危险命令模式、版本号格式与递增；并提供技能被修改或从备份仓库恢复后自动执行审核的通道（结论作为上下文回传给模型）。当技能被新建/修改/从备份恢复、技能里的脚本改过、多机同步技能之后，或需要回答『这个技能是否可用、有没有把密钥写进去、引用的脚本还在不在』时使用。触发词：技能审核、skill audit、审核技能、技能体检、技能改动后检查、恢复技能后检查、技能脚本没BOM、技能引用失效、技能泄漏密钥。"
...
| V1 | 版本号格式 | fail | `version` 必须三段式 `主.次.修`（形如 `1.2.3`）。缺 `version` 仍由 F1 报 warn；格式不符是**明确契约违规**，fail 级不可豁免 |
| V2 | 版本号递增 | warn | 技能位于 **git 工作副本**、且 `SKILL.md` 有未提交改动时，与 `git HEAD` 那一版比较：降级 = 倒退；同级而内容有实质改动 = 忘了 bump（**忽略全部空白**后内容相同视为纯排版，不报）。取不到 git 上下文（非 git 目录 / 已提交 / 新文件 / git 不可用）一律**静默跳过** |

**V1/V2 只管「格式」与「递增」，不管「分级」**：`重构升第一级 / 新功能升第二级 / 修 bug 升第三级` 是对**改动性质**的判断，静态引擎看不到"这次改的是什么"，硬猜必然误报——那条规则的真源在用户全局指令里，由人在改技能时判定。引擎只把守两件可判定的：格式一律三段式（V1）、相对上一版不得倒退且实质改动须 bump（V2）。V2 的"上一版"取自 git `HEAD`，因此它**只在编辑技能的过程中**（工作区相对 HEAD 有未提交改动）生效；已提交内容、非 git 目录、新文件一律沉默——宁可漏报，不可误报。
...
- **形式**：`<!-- audit:ignore <代码> <目标> <理由> -->`；代码为 `F1/S1/R1/V1/V2/X1/X2/X3` 之一，或 `*`（全部）。fail 级代码（`V1`、`S1`、`X1` 及 F1/R1 的 fail 分支）**一律不可豁免**，列出只为形态完整。
