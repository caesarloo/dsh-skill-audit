...
| 审核引擎 `scripts/audit-skills.ps1` | 只有**文件系统**（技能目录里的字节） | 一切**静态可判定**的判据（frontmatter、BOM、引用、凭据、危险命令） | 改完即生效；所有通道同时受益。引擎住在技能根下一个**没有 `SKILL.md`** 的目录里——那是它的家，不是技能 |
...
- **触发规则**：**写入类文件工具**（`write`/`edit`/`multi_edit`/`notebook_edit`/`apply_patch` …）命中 `<DSH_HOME>/skills/<技能>/` → 只审该技能；但命中的目录**没有 `SKILL.md`**（= 判据引擎真源的家）→ **全量**（判据本身变了，定向无从谈起：只审一个不是技能的东西毫无意义）；**任何以 `mode: restore` / `mode: backup` 调用的工具** → 全量（判据是**调用形态**而不是工具名，因此不绑定任何具体备份插件——没有装备份插件的用户也不会因此产生死逻辑）；`pwsh` 等 shell 的命令行同时含 `skills` 与写操作迹象（`Set-Content`/`Copy-Item`/`Remove-Item`/`robocopy` …）→ 全量；其它 → 静默。**只读工具（`read`/`glob`/`grep`）刻意不触发**——它们同样携带 `file_path` 却不改内容；不加这条白名单，每读一次技能文件就会注入一次审核上下文。
...
- **引擎单一真源**：插件不含审核规则，只负责"触发 + 回传"；规则始终在 `scripts/audit-skills.ps1`。**但引擎不一定住在本技能目录里**——本技能正文随包发布（不常改），而引擎按 `config.auditScript` > 用户态技能目录 > 包内副本 的优先级解析：常改的判据留在用户态技能目录里，改完即生效、不必发版。实际用的是哪一个，以 §三 手工通道给出的路径为准。
